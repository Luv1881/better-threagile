...
To additionaly generate the adoc report you have to run threagile with the option `--skip-report-pdf` (the adoc report is generated by default) then a folder adocReport will be generated inside the output folder. After that you can run your favorite asciidoctor command to build a pdf or html page or whatever you want from it. The below example uses the asciidoctor docker container to create a pdf.
