...
func Import(data []byte, opts ImportOptions) (*types.Model, error) {
	...
	// Kustomize-style overlay directories repeat the same workloads; keep only
	// the first occurrence of each generated asset ID, or the same asset ends up
	// listed several times in its namespace boundary (which the analyzer rejects
	// as "modeled in multiple trust boundaries").
	b.workloads = dedupeWorkloads(b.workloads)
...
func (b *builder) buildWorkloadAsset(w *workload) {
	...
	if assetType == types.Datastore {
		// Datastores store data by definition; the stub lets the storage and
		// encryption rules fire on bootstrapped models.
		asset.DataAssetsStored = appendUnique(asset.DataAssetsStored, b.storedDataStub())
	}
	b.model.TechnicalAssets[asset.Id] = asset
}

// storedDataStub returns the generic "imported data" stub data asset id,
// creating the asset on first use.
func (b *builder) storedDataStub() string {
	id := toID("data-imported", b.label)
	if b.model.DataAssets[id] == nil {
		b.model.DataAssets[id] = &types.DataAsset{
			Id:              id,
			Title:           "Imported Data (review and classify)",
			Description:     "Stub data asset generated by kubernetes import.",
			Confidentiality: types.Confidential,
			Integrity:       types.Critical,
			Availability:    types.Critical,
		}
...
func (b *builder) markInternetExposed(targetAssetID, clientID, title string, protocol types.Protocol) {
	...
	if target.Type == types.Datastore {
...
func (b *builder) assignNamespaceBoundaries() {
	...
	addAsset := func(namespace, assetID string) {
		if seen[namespace] == nil {
			seen[namespace] = map[string]bool{}
		}
		if seen[namespace][assetID] {
			return
		}
		seen[namespace][assetID] = true
		byNamespace[namespace] = append(byNamespace[namespace], assetID)
	}
	for _, w := range b.workloads {
		addAsset(w.namespace, w.assetID)
	}
...
// dedupeWorkloads keeps the first workload per generated asset ID. Overlay
// directories (kustomize dev/prod/observability trees) redeclare the same
// workloads, and without this the boundary assignment lists an asset more than
// once, which the analyzer rejects.
func dedupeWorkloads(workloads []*workload) []*workload {
	seen := map[string]bool{}
	out := workloads[:0]
